#!/usr/bin/env node
/**
 * Jira CLI — connects to the EID board on hongleigu19.atlassian.net
 *
 * Usage:
 ...
 *   issue <KEY>        Show details for a specific issue (e.g. EID-1)
 */
...
async function cmdBoard() {
  ...
  console.log(`  URL  : ${BASE_URL}/jira/software/projects/EID/boards/${BOARD_ID}\n`);
}
...
async function cmdIssues(statusFilter?: string) {
  ...
  if (statusFilter) {
    jql = `project = EID AND status = "${statusFilter}" ORDER BY updated DESC`;
  } else {
    jql = `project = EID AND status != Done ORDER BY updated DESC`;
  }
...
async function cmdIssue(key: string) {
  const normalised = key.toUpperCase().startsWith("EID") ? key.toUpperCase() : `EID-${key}`;
...
${c.bold("Commands:")}
  ...
  issue <EID-N>        Show issue details
`);
}
  ...
  try {
    switch (cmd) {
      ...
      case "issues":
        ...
        break;
      case "issue":
        if (!arg) { console.error("Usage: jira issue <EID-N>"); process.exit(1); }
        await cmdIssue(arg);
        break;
      default:
        help();
